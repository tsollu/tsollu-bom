...
import com.kaddo.exception.AssertUtil;
...
public class PageQuery extends Query {
    ...
    public PageQuery setPageNum(Integer pageNum) {
        this.pageNum = Objects.isNull(pageNum) ? 1 : pageNum;
        return this;
    }
     ...
     * @throws com.kaddo.exception.ValidationException 日期格式解析错误则抛出异常
     */
    public Date getBeginTime() {
        if (StrUtil.isNotBlank(beginTime)) {
            try {
                ...
                String reason = StrUtil.format("日期格式解析错误：beginTime={}, error={}", beginTime, e.getMessage());
                AssertUtil.validate(reason).assertThrow();
            }
        }
        return null;
    }
    ...
    public Date getEndTime() {
        if (StrUtil.isNotBlank(endTime)) {
            try {
                ...
                String reason = StrUtil.format("日期格式解析错误：endTime={}, error={}", endTime, e.getMessage());
                AssertUtil.validate(reason).assertThrow();
            }
        }
        return null;
