package com.kaddo.framework.aspectj;

import com.google.common.base.Throwables;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.serializer.SerializerFeature;
import com.kaddo.framework.KaddoConstants;
import com.kaddo.framework.annotation.CatchAndLog;
import com.kaddo.framework.aop.AfterAdvice;
import com.kaddo.framework.aop.AfterThrowingAdvice;
import com.kaddo.framework.aop.AroundAdvice;
...
public class CatchAndLogAspect implements AfterAdvice, AroundAdvice, AfterThrowingAdvice, WebMvcConfigurer {
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        registry.addInterceptor(new HandlerInterceptor() {
            ...
            public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) throws Exception {
                MDC.remove(KaddoConstants.LOG_TRACE_ID); // WEB请求执行完成后，清除当前线程的MDC数据
            }
        }).addPathPatterns("/**");
    }

    @Override
    @Pointcut("@annotation(com.kaddo.framework.annotation.CatchAndLog) && execution(public * *(..))")
    ...
    public void after(JoinPoint joinPoint) {
        MDC.remove(KaddoConstants.LOG_TRACE_ID); // 方法执行完成后，清除当前线程的MDC数据
    }
